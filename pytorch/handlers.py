...
def handle_model(args, device):
    ...
    # criterion
    criterion = nn.CrossEntropyLoss() if args.no_cosine else CriterionHandler(args, args.iterations).to(device)
...
def load_from_file(file, model=None, model_ema=None, optimizer=None, scheduler=None, scaler=None):
    ...
    return model, model_ema, train_metrics, test_metrics, \
          optimizer, scheduler, scaler, iteration
...
def calculate_norms(model, first_logits, second_logits, first_features, second_features):
    return {
            ...
            "first-feature-norm-1":  torch.linalg.vector_norm(first_features, ord=1, dim=1).mean(),
            "first-feature-norm-2": torch.linalg.vector_norm(first_features, ord=2, dim=1).mean(),
            "first-feature-norm-inf" : torch.linalg.vector_norm(first_features, ord=float('inf'), dim=1).mean(),
            "second-feature-norm-1" : torch.linalg.vector_norm(second_features, ord=1, dim=1).mean(),
            "second-feature-norm-2" : torch.linalg.vector_norm(second_features, ord=2, dim=1).mean(),
            "second-feature-norm-inf" : torch.linalg.vector_norm(second_features, ord=float('inf'), dim=1).mean(),
            "logit-divergence" : jensen_shannon(first_logits, second_logits), 
            "feature-cosine" : nn.functional.cosine_similarity(first_features, second_features).mean(),
            "feature-l2" : nn.functional.mse_loss(first_features, second_features),
            "feature-l1" : nn.functional.l1_loss(first_features, second_features),
           }
