...
from evaluation import jensen_shannon

@torch.no_grad()
def watch_under_transforms(model, test_set, device=torch.device("cuda:0"), args):
    base_transform = test_set.transform
    ...
    for transform in args.transforms_to_track.split(","):
        ...
        test_set.transform = loaded_transform
        
        softmaxes, preds, labels, loss, features = predict(model, dataset, test_set, device=device)
    ...
    result = {}
    cosine, l2, l1 = nn.CosineSimilarity(), nn.MSE(), nn.L1Loss()
    ...
    for i in cache:
        for j in cache:
            if i != j and (i + "-" j) not in result:
                key = i + "-" j
                result[key] = {
                        "softmax-divergence" : jensen_shannon( cache[i]["softmaxes"],  cache[j]["softmaxes"],),
                        "cosine" : cosine(cache[i]["features"], cache[j]["features"]).mean(),
                        "l2" : l2(cache[i]["features"], cache[j]["features"]).mean(),
                        "l1" : l1(cache[i]["features"], cache[j]["features"]).mean(),
                        "pred-overlap" : (cache[i]["preds"] == cache[j]["preds"]).sum() / len(dataset.dataset),
                        "true-pred-overlap" : torch.logical_and( 
                                                (cache[i]["preds"] == cache[i]["labels"]),
                                                (cache[j]["preds"] == cache[j]["labels"])
                                              ).sum() / len(dataset.dataset),
