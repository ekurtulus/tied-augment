...
def reduced_cifar(root, cifar_type, num_labels, train=False, transform=None):
    ...
    data = datasets.CIFAR10(root) if cifar_type == "cifar10" else datasets.CIFAR100(root)
    ...
    for j, i in enumerate(targets):
        if sampled_count[i] < per_class:
            ...
            sampled_labels.append( targets[j] )
            sampled_count[i] += 1
    
    return BasicDataset(sampled_images, sampled_targets)
...
DATASETS = {
    ...
    "reduced_cifar10" : {
        "dataset_function" : partial(reduced_cifar, cifar_type="cifar10", 4000),
        "mean" : torch.tensor([0.4914, 0.4822, 0.4465]),
        "std" : torch.tensor([0.2475, 0.2435, 0.2615]),
        "num_classes" : 10,
        "split_args" : {"train" : {"train" : True}, "test" : {"train" : False}},
    ...
    "reduced_cifar100" : {
        "dataset_function" : partial(reduced_cifar, cifar_type="cifar100", 4000),
        "mean" : torch.tensor([0.5070, 0.4865, 0.4409]),
        "std" : torch.tensor([0.2673, 0.2564, 0.2761]),
        "num_classes" : 100,
        "split_args" : {"train" : {"train" : True}, "test" : {"train" : False}},
...
def load_dataset(args):
    ...
    train_data = TwoBranchDataset( dataset_function(root=args.datapath, **split_args["train"], transform=None),
                                  first_transform, second_transform, use_same_crop=args.use_same_crop )    
