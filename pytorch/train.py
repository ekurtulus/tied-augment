...
def normal_step(batch, model, criterion, device, step, optimizer, args):
    ...
    if step == "baseline":
        first_logits = model(x)[1]
        second_logits = first_features = second_features = None 
        
    elif step == "tied-augment" and args.single_forward:
    ...
    loss = criterion(first_logits, second_logits, first_features, second_features, y,
                                     ce=True, contrastive=(step != "baseline"))    
    loss.backward()
    optimizer.step()
...
def train(args, train_set=None, test_set=None, clean_train_set=None,
    ...
    for iteration in range(args.start_iteration, args.iterations + 1):
        ...
        if args.track_test and iteration in args.iterations_to_test:
            ...
            writing_temp = {}
            for key, item in watch_under_transforms(model, test_set, device=device, args).items():
        ...
        if iteration % args.epoch_every == 0 or iteration == args.iterations:
            ...
            test_softmaxes, test_preds, test_labels, test_loss = predict(model.module if args.model_ema else model, test_set, device=device, halftensor=halftensor)
    ...
    results = all_evaluate(model, clean_train_set, test_set, args.robustness_path, args.mean, args.std, device=device, halftensor=halftensor,
...
def run(args):
    ...
    if args.track_test:
        if args.step_function_scheduler == "linear":
            args.__dict__["iterations_to_test"] = np.linspace(0, args.iterations, args.num_tests) 
        elif args.step_function_scheduler == "log":
            args.__dict__["iterations_to_test"] = np.geomspace(0, args.iterations, args.num_tests)
    
    results = train_function(args, train_set=train_set, test_set=test_set, clean_train_set=clean_train_set,
                             model=model, model_ema=model_ema, optimizer=optimizer, 
...
if __name__ == "__main__":
    ...
    # contrastive args
    parser.add_argument("--contrastive_loss", type=str, default="cosine", choices=["cosine", "vicreg", "cka", "mse", "mae",
    ...
    parser.add_argument("--transforms_to_track")
