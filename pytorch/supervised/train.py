...
def normal_step(batch, model, criterion, device, step, optimizer, args, sam=False, sam_step="first"):
    ...
    if step == "baseline":
        model.eval()
        with torch.no_grad():
            second_logits, second_features = model(second, return_features=True)
        model.zero_grad()
        
        model.train()
        first_logits, first_features = model(first)
...
def run(args):
    ...
    results = train(args, train_set=train_set, test_set=test_set, clean_train_set=clean_train_set,
                             model=model, model_ema=model_ema, optimizer=optimizer, 
